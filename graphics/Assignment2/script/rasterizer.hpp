...
namespace rst
{
    ...
    private:
        ...
        std::vector<Eigen::Vector3f> color_frame_buf;

        std::vector<float> depth_buf;
        std::vector<float> color_depth_buf;
        float get_index(float x, float y);
        float get_new_index(float x, float y);
